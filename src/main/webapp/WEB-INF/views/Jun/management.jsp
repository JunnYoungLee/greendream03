...
<div>
	...
	<c:if test="${paging.prev}">
		<a href="management?pageNum=${paging.startPage-1}&amount=${paging.cri.amount}">이전</a>
	</c:if>
	<c:if test="${sessionScope.yldto.dept eq '재무과'}">
	<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="num">
		<a href="management?pageNum=${num}&amount=${paging.cri.amount}">${num}</a>
	</c:forEach>
	</c:if>
	<c:if test="${paging.next}">
		<a href="management?pageNum=${paging.endPage+1}&amount=${paging.cri.amount}">다음</a>
</c:if>
</div>
